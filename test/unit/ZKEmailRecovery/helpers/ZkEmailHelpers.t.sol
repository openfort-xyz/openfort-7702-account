...
import {
    IGuardianManager
} from "zk-email-recovery-contracts/email-recovery/src/interfaces/IGuardianManager.sol";
import {
    GuardianStorage,
    GuardianStatus
...
abstract contract ZkEmailHelpers is ProoverHelpers, IKey {
    ...
    function _computeGuardianAddress(address _owner, bytes32 _salt)
        internal
        view
        returns (address)
    {
    ...
    function _createInstallDataWithCustomSelector(
        ...
        bytes memory isInstalledContext = bytes("");
        bytes4 functionSelector = OPF7702Recoverable.completeRecovery.selector;
    ...
    function _createInstallDataForKeyRecovery(
        ...
        bytes memory isInstalledContext = bytes("");
        // Use MockValidator.startRecovery.selector for Key-based recovery
        bytes4 functionSelector = ERC7579Module.completeRecovery.selector;
        ...
        )
    {
        ...
        IGuardianManager.GuardianConfig memory config =
            universalEmailRecoveryModule.getGuardianConfig(account);

        return (config.guardianCount, config.totalWeight, config.acceptedWeight, config.threshold);
    }

    function _getGuardianStatus(address account, address guardian)
        internal
        view
        returns (uint256 status, uint256 weight)
    {
        ...
        GuardianStorage memory guardianStorage =
            universalEmailRecoveryModule.getGuardian(account, guardian);
     ...
     */
    function _computeRecoveryDataHash(address _validator, address _newOwner)
        internal
        pure
     ...
     * @return recoveryDataHash The keccak256 hash
     */
    function _computeRecoveryDataHashForKey(address _validator, KeyDataReg memory _newOwnerKey)
        internal
        pure
        returns (bytes memory recoveryData, bytes32 recoveryDataHash)
    {
        ...
        bytes memory recoveryCallData =
            abi.encodeWithSelector(OPF7702Recoverable.completeRecovery.selector, _newOwnerKey);
     ...
     * @return recoveryDataHash The keccak256 hash
     */
    function _computeRecoveryDataHashForSwapOwner(address _validator, address _newOwner)
        internal
        pure
        returns (bytes memory recoveryData, bytes32 recoveryDataHash)
    {
    ...
    function _buildRecoveryEmailAuthMsg(
        ...
        string memory maskedCommand = string.concat(
            ...
            " using recovery hash ",
            _bytes32ToString(_recoveryDataHash)
        );
     ...
     */
    function _buildRecoveryEmailAuthMsgGuardian1(address _account, bytes32 _recoveryDataHash)
        internal
        view
     ...
     */
    function _buildRecoveryEmailAuthMsgGuardian2(address _account, bytes32 _recoveryDataHash)
        internal
        view
        returns (EmailAuthMsg memory)
    {
