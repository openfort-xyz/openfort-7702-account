...
abstract contract RecoveryProofs is Proofs {
    ...
    string public json_proofs_key_eoa_recovery =
        vm.readFile("test/unit/ZKEmailRecovery/data/proofs/RecoveryProofsData.json");
    string public json_proofs_key_eoa_indi_recovery =
        vm.readFile("test/unit/ZKEmailRecovery/data/proofs/RecoveryProofsDataEOA_Indi.json");
    ...
    function _loadAllRecoveryProofs(ProofType _proofType) internal {
        if (_proofType == ProofType.SAFE) {
            ...
            path_recovery = json_proofs_key_eoa_indi_recovery;
        }
