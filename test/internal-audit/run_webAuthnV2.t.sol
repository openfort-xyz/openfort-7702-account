...
contract RunWebAuthnV2 is Base {
    ...
    function setUp() public {
        ...
        SEPOLIA_WEBAUTHN_V2 = new WebAuthnVerifierV2();
        webAuthn = WebAuthnVerifier(payable(address(SEPOLIA_WEBAUTHN_V2)));

        _createInitialGuradian();
        /* deploy implementation & bake it into `owner` address */
        implementation = new OPF7702(
            address(entryPoint),
            address(SEPOLIA_WEBAUTHN_V2),
            RECOVERY_PERIOD,
            LOCK_PERIOD,
            SECURITY_PERIOD,
            SECURITY_WINDOW
    ...
    function test_RegisterKeyEOAWithMKV2() public {
        ...
        IKey.PubKey memory pubKeyExecuteBatch =
            IKey.PubKey({x: REG_PUBLIC_KEY_X_V2, y: REG_PUBLIC_KEY_Y_V2});

        bytes memory _signature = account.encodeWebAuthnSignature(
            true,
            REG_AUTHENTICATOR_DATA_V2,
            REG_CLIENT_DATA_JSON_V2,
            REG_CHALLENGE_INDEX_V2,
            REG_TYPE_INDEX_V2,
            REG_SIGNATURE_R_V2,
            REG_SIGNATURE_S_V2,
            pubKeyExecuteBatch
    ...
    function _initializeAccount() internal {
        ...
        pubKeyMK = PubKey({x: REG_PUBLIC_KEY_X_V2, y: REG_PUBLIC_KEY_Y_V2});
