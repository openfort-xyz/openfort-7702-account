...
abstract contract Execution is KeysManager, ReentrancyGuard {
    ...
    error TooManyCalls(uint256 total, uint256 max);

    /// @dev Thrown when `opData`‑aware mode is requested but not yet
    ///      implemented.
    error UnsupportedOpData();
    ...
    function execute(bytes32 mode, bytes memory executionData)
        ...
        payable
        virtual
        nonReentrant // ← re‑entrancy blocked once here
    {
    ...
    function _run(bytes32 mode, bytes memory data, uint256 counter) internal returns (uint256) {
        ...
        if (id == 0) revert IExecution.UnsupportedExecutionMode();
        ...
        if (opData.length != 0) revert UnsupportedOpData();
        ...
        for (uint256 i; i < calls.length; ++i) {
            ...
            if (++counter > MAX_TX) revert TooManyCalls(counter, MAX_TX);
        }
        return counter;
    }
