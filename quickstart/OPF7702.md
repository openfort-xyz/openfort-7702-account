<h1 align="center">Openfort EIP-7702 Smart Accounts Quickstart</h1>
...
- Multi-scheme signature support (EOA, WebAuthn, P‑256, non-extractable P‑256) with automatic dispatch inside `BaseOPF7702`.
- Session keys governed by `KeysManager`: target/function whitelists, spend caps, and custodial gas policy integration.
- Guardian-based recovery via `OPF7702Recoverable` and `SocialRecoveryManager`, including wallet locks and quorum-based execution.
- ERC-7821 style batch execution with bounded recursion and call-count enforcement.
- Optional per-session gas/cost accounting through `GasPolicy` to mitigate griefing during sponsored operations.
- Deterministic storage layout (ERC-7201) for safe upgrades and delegation.
...
const SEPOLIA_RPC = '';
const authorizer = privateKeyToAccount('0x...');
...
```
* Source: [OPF7702Recoverable](https://github.com/openfort/openfort-7702-account/blob/main/src/core/OPF7702Recoverable.sol)
...
* Prepare Key Data
```ts
import { zeroAddress, toHex, keccak256, encodeAbiParameters, encodeFunctionData } from "viem";

export enum KeyType {
  EOA = 0,
  WEBAUTHN = 1,
  P256 = 2,
  P256_NON_KEY = 3, // corresponds to IKey.KeyType.P256NONKEY
}
...
> Granting function/target permissions and spend limits is performed after initialization via `setCanCall`, `setTokenSpend`, and related `KeysManager` helpers. The registration payload focuses on key metadata.
...
const sessionKeyBytes = encodeAbiParameters(
  [{ type: "bytes32" }, { type: "bytes32" }],
  [toHex(publicKey.x), toHex(publicKey.y)]
);
```
...
const sessionKeyReg = {
  ...
  limits: ACTIONS_LIMIT,
  key: sessionKeyBytes,
  keyControl: KeyControl.Custodial,
} as const;

// Configure permissions after registration, e.g.:
// await account.setCanCall(keyId, target, selector, true);
// await account.setTokenSpend(keyId, token, allowance, period);
```
<br></br>
* Call Data (If Sponsored use with `userOperation`) 
```ts
